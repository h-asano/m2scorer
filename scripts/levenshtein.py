...
def f1_suffstats(candidate, source, gold_edits, max_unchanged_words=2, ignore_whitespace_casing=False, verbose=False, very_verbose=False):
    ...
    if ignore_whitespace_casing:
        editSeq = [x for x in editSeq if not equals_ignore_whitespace_casing(x[2], x[
                                                                             3])]
...
def batch_multi_pre_rec_f1(candidates, sources, gold_edits, max_unchanged_words=2, beta=0.5, ignore_whitespace_casing=False, verbose=False, very_verbose=False, sentence_level=False, use_skip=False, maxlen_v=1000):
    ...
    for candidate, source, golds_set in zip(candidates, sources, gold_edits):
        ...
        if use_skip and (len(V1) > maxlen_v and len(V2) > maxlen_v):
            if verbose:
                print(">> Skipping line", i, ", len(V1):",
                      len(V1), "len(V2):", len(V2))
            if sentence_level:
                print float(0)
            continue
        ...
        # Find measures maximizing current cumulative F1; local: curent
        # annotator only
        sqbeta = beta * beta
        chosen_ann = -1
        f1_max = -1.0
        ...
        for annotator, gold in golds_set.iteritems():
            localdist = set_weights(
                E, dist, edits, gold, verbose, very_verbose)
            ...
            correct = matchSeq(
                editSeq, gold, ignore_whitespace_casing, verbose)
...
def batch_multi_pre_rec_f1_sub(candidate, source, golds_set, max_unchanged_words, beta, ignore_whitespace_casing, verbose, very_verbose, i, sentence_level=False, use_skip=False, maxlen_v=5000):
    ...
    #V, E, dist, edits = edit_graph(lmatrix, backpointers)
    V1, E1, dist1, edits1 = edit_graph(lmatrix1, backpointers1)
    if use_skip and len(V1) > maxlen_v:
        if verbose:
            print(">> Skipping line", i, ", len(V1):", len(V1))
        if sentence_level:
            print(i, float(0))
        return stat_correct, stat_proposed, stat_gold
    V2, E2, dist2, edits2 = edit_graph(lmatrix2, backpointers2)
    ...
    for annotator, gold in golds_set.iteritems():
        ...
        stat_correct_local = len(correct)
        stat_proposed_local = len(editSeq)
        stat_gold_local = len(gold)
        p_local = comp_p(stat_correct_local, stat_proposed_local)
        r_local = comp_r(stat_correct_local, stat_gold_local)
        f1_local = comp_f1(stat_correct_local,
                           stat_proposed_local, stat_gold_local, beta)
...
def batch_pre_rec_f1(candidates, sources, gold_edits, max_unchanged_words=2, beta=0.5, ignore_whitespace_casing=False, verbose=False, very_verbose=False):
    ...
    for candidate, source, gold in zip(candidates, sources, gold_edits):
        ...
        if verbose:
            ...
            print("f_%.1f          :" % beta, comp_f1(
                stat_correct, stat_proposed, stat_gold, beta))
            print("-------------------------------------------")
...
def shrinkEdit(edit):
    ...
    while j <= min(len(origtok), len(corrtok)) - cstart and not found:
        if origtok[len(origtok) - j] != corrtok[len(corrtok) - j]:
            ...
            cend -= 1
            j += 1
    shrunkEdit = (edit[0] + i, edit[1] - (j - 1), ' '.join(origtok[i: len(
        origtok) - (j - 1)]), ' '.join(corrtok[i: len(corrtok) - (j - 1)]))
    return shrunkEdit
    ...
    for e in reversed(editSeq):
        for i in range(last_index, len(goldSeq)):
            ...
            if matchEdit(e, g, ignore_whitespace_casing):
                ...
                last_index = i + 1
                if verbose:
                    nextEditList = [shrinkEdit(edit)
                                    for edit in editSeq if e[1] == edit[0]]
                    prevEditList = [shrinkEdit(edit)
                                    for edit in editSeq if e[0] == edit[1]]
                    ...
                    if e[0] != e[1]:
                        ...
                        nextEditList = [edit for edit in nextEditList if edit[
                            0] < edit[1] and edit[3] == '']
                        prevEditList = [edit for edit in prevEditList if edit[
                            0] < edit[1] and edit[3] == '']

                    matchAdj = any(any(matchEdit(edit, gold, ignore_whitespace_casing) for gold in goldSeq) for edit in nextEditList) or \
                        any(any(matchEdit(edit, gold, ignore_whitespace_casing)
                                for gold in goldSeq) for edit in prevEditList)
    ...
    for k in sorted(M.keys()):
        ...
        if k[0] == k[1]:  # insertion case
            ...
            while lptr <= rptr:
                ...
                edge = M[k][cur]
                thisEdit = edits[edge]
                # only check start offset, end offset, original string,
                # corrections
                if very_verbose:
...
def merge_edits(e1, e2, joiner=' '):
    if e1[0] == 'ins':
        if e2[0] == 'ins':
            e = ('ins', e1[1], e2[2], '', e1[4] +
                 joiner + e2[4], e1[5] + e2[5])
        elif e2[0] == 'del':
            e = ('sub', e1[1], e2[2], e2[3], e1[4], e1[5] + e2[5])
        elif e2[0] == 'sub':
            e = ('sub', e1[1], e2[2], e2[3], e1[
                 4] + joiner + e2[4], e1[5] + e2[5])
        elif e2[0] == 'noop':
            e = ('sub', e1[1], e2[2], e2[3], e1[
                 4] + joiner + e2[4], e1[5] + e2[5])
    elif e1[0] == 'del':
        if e2[0] == 'ins':
            ...
            e = ('del', e1[1], e2[2], e1[3] +
                 joiner + e2[3], '', e1[5] + e2[5])
        elif e2[0] == 'sub':
            e = ('sub', e1[1], e2[2], e1[3] +
                 joiner + e2[3], e2[4], e1[5] + e2[5])
        elif e2[0] == 'noop':
            e = ('sub', e1[1], e2[2], e1[3] +
                 joiner + e2[3], e2[4], e1[5] + e2[5])
    elif e1[0] == 'sub':
        if e2[0] == 'ins':
            e = ('sub', e1[1], e2[2], e1[3], e1[
                 4] + joiner + e2[4], e1[5] + e2[5])
        elif e2[0] == 'del':
            e = ('sub', e1[1], e2[2], e1[3] +
                 joiner + e2[3], e1[4], e1[5] + e2[5])
        elif e2[0] == 'sub':
            e = ('sub', e1[1], e2[2], e1[3] + joiner + e2[3],
        ...
        if e2[0] == 'ins':
            ...
            e = ('noop', e1[1], e2[2], e1[3] + joiner +
                 e2[3], e1[4] + joiner + e2[4], e1[5] + e2[5])
    else:
        assert False
    return e
